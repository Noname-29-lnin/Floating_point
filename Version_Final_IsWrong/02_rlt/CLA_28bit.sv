module CLA_28bit(
    ...
    genvar i;
    generate
        for (i = 0; i < 7; i = i + 1) begin : CLA_BLOCK_GEN
    ...
    assign w_c[3] = w_g[2] | (w_p[2] & w_g[1]) | (w_p[2] & w_p[1] & w_g[0]) 
                             | (w_p[2] & w_p[1] & w_p[0] & w_c[0]);
    assign w_c[4] = w_g[3] | (w_p[3] & w_g[2]) | (w_p[3] & w_p[2] & w_g[1]) 
                             | (w_p[3] & w_p[2] & w_p[1] & w_g[0])
                             | (w_p[3] & w_p[2] & w_p[1] & w_p[0] & w_c[0]);
    assign w_c[5] = w_g[4] | (w_p[4] & w_g[3]) | (w_p[4] & w_p[3] & w_g[2])
                             | (w_p[4] & w_p[3] & w_p[2] & w_g[1])
                             | (w_p[4] & w_p[3] & w_p[2] & w_p[1] & w_g[0])
                             | (w_p[4] & w_p[3] & w_p[2] & w_p[1] & w_p[0] & w_c[0]);
    assign w_c[6] = w_g[5] | (w_p[5] & w_g[4]) | (w_p[5] & w_p[4] & w_g[3])
    ...
    assign w_c[7] = w_g[6] | (w_p[6] & w_g[5]) | (w_p[6] & w_p[5] & w_g[4])
                             | (w_p[6] & w_p[5] & w_p[4] & w_g[3])
