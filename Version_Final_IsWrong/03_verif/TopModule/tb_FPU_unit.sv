...
function automatic shortreal cal_rounding_error(
    ...
    begin
        ...
        if (s_is_nan) begin
            cal_rounding_error = (s_is_nan == e_is_nan) ? 0.0 : 100.0;
        end else if (s_is_inf) begin
            cal_rounding_error = (s_bits == e_bits) ? 0.0 : 100.0;
...
endtask
task automatic Display_result_V2(
    string                      t_type      ,
    input logic                 t_i_add_sub ,
    input logic [31:0]          t_i_32_a    ,
    input logic [31:0]          t_i_32_b    ,
    ...
    t_sr_32_s   = float_to_real(t_o_32_s);
    t_sr_32_e   = check_functional(t_i_add_sub, t_i_32_a, t_i_32_b);
    t_w_32_s    = real_to_float(t_sr_32_e);
    begin
        $display("[%s][%s]i_32_a=%h (%.24f) %s i_32_b=%h (%.24f) \t| o_32_s=%h (%.24f) \t| o_ov_flow=%b, o_un_flow=%b",
                    t_type, (t_i_add_sub == 1'b1) ? "SUB" : "ADD", 
                    t_i_32_a, t_sr_32_a, (t_i_add_sub == 1'b1) ? "-" : "+", t_i_32_b, t_sr_32_b, t_o_32_s, t_sr_32_s,
                    t_o_ov_flow, t_o_un_flow);
        $display("=> %s: expect=%.24f (%h), dut=%.24f (%h), rounding_error=%.8f %% (exp_error = %.8f %%)", 
                    (t_o_32_s == t_w_32_s) ? "PASS" : "FAIL", 
                    t_sr_32_e, real_to_float(t_sr_32_e), t_sr_32_s, real_to_float(t_sr_32_s), t_sr_rounding_error, t_error);
        test_count++;
        if (t_o_32_s == t_w_32_s) test_pass++;
    end
endtask //automatic
task automatic TestCase_Display_result(
    ...
    begin
        ...
        // Display_result_V2(t_type, i_add_sub, i_32_a, i_32_b, o_32_s, o_ov_flow, o_un_flow);
        @(posedge i_clk);
        #1;
        i_add_sub   = 1'b0;
        i_32_a      = t_i_fpu_b;
